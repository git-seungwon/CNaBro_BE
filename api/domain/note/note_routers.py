...
from api.domain.user import user_crud, user_routers
from api.models import tasks

from api.domain.note import note_crud, note_schemas
...
# 전체 노트 조회
@router.get("/notes", response_model=note_schemas.NoteList, tags=["notes"])
async def note_list(db: note_schemas = Depends(get_db), page: int = 0, size: int = 10, keyword: str = '',
                    current_user:tasks.User = Depends(user_routers.get_current_user)):
...
# 노트 조회
@router.get("/notes/{note_id}", response_model=note_schemas.Notes, tags=["notes"])
...
async def note_create(_note_create: note_schemas.NoteCreate, db: AsyncSession = Depends(get_db),
                          current_user:tasks.User = Depends(user_routers.get_current_user)):
...
async def note_update(_note_update: note_schemas.NoteUpdate, db: AsyncSession = Depends(get_db),
                          current_user:tasks.User = Depends(user_routers.get_current_user)):
    db_note = await note_crud.get_note(db, note_id=_note_update.note_id)
    if not db_note:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail="데이터를 찾을 수 없습니다.")
    if current_user.user_id != db_note.user.user_id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail="수정 권한이 없습니다.")
...
async def note_delete(_note_delete: note_schemas.NoteDelete, db: AsyncSession = Depends(get_db),
                          current_user:tasks.User = Depends(user_routers.get_current_user)):
